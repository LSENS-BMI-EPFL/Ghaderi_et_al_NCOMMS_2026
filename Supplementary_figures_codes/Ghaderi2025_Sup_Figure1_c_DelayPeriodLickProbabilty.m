%% =========================================================================
% Ghaderi2025_Figure1Sup_DelayPerioLickProbabilty.m
% =========================================================================
% 
% This script generates Figure 1 Supplementary showing delay period lick probability
...
% Date: 2025
%
% Description: This script analyzes and plots lick probability during the delay 
% period across different trial types. The analysis focuses on early lick behavior 
% and calculates mean probability with standard error across sessions.
%
% Dependencies: 
...
%
% Output: PDF figure showing delay period lick probability analysis with proper formatting
% =========================================================================
...
params.completion_state = 'early_licks';  % Options: 'early_licks', 'completed_trials', 'all_trials'
...
%% Calculate lick probability for each trial type
for icond = 1:length(params.TrialType)
    ...
    for i = unique_session_ids'
        ...
        % Calculate lick probability for current session and trial type

        plick = sum(completion_state & IndTrialType) / sum(IndTrialType) * 100;
        performance(icond, session_counter) = plick;
        session_counter = session_counter + 1;

    end % End of session loop
    
    % Plot performance for current trial type
    hold(axs(1), "on");
    bar(axs(1), icond, nanmean(performance(icond, :)), 'FaceColor', colorcodes(icond, :));
    errorbar(axs(1), icond, nanmean(performance(icond, :)), nanstd(performance(icond, :)) / sqrt(sum(~isnan(performance(icond, :)))), 'vertical', 'Color', [0 0 0], 'CapSize', 10);
end % End condition loop

%% Plot individual session data
plot(axs(1), performance, 'o', 'color', [0 0 0 .2]);
...
ylabel(axs(1), 'Lick probability (%)');  % Note: This appears to be a typo in the original - should be "Lick probability (%)"
...
% Alternative export options (commented out)
