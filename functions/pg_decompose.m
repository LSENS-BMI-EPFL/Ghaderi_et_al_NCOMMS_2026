function [rez]=pg_decompose(Data)
% clear,clc,close all
...
%              Data.motionEnergy using a manually set threshold
%   anm/date - session meta data

nBins = size(Data.seq,1);
nTrials = size(Data.seq,2);
nNeurons = size(Data.seq,3);

%% PREPROCESS DATA
% choice of normalization/standardization is up to you, here just zscoring

temp = reshape(Data.seq,nBins*nTrials,nNeurons);
...
moveMask = Data.moveMask(:); %(time bins * nTrials)
...
% sortTimes = [0.8 1]; % in seconds, relative to go cue
% for i = 1:numel(sortTimes)
%     closest_val = interp1(Data.time,Data.time,sortTimes(i),'nearest');
%     ix(i) = find(Data.time==closest_val);
% end
% [~,sortix] = sort(mean(Data.motionEnergy(ix(1):ix(2),:),1)); % sorted trial idx
% 
% 
% % plot motion energy
% ax = subplot(1,3,1);
...
% hold on;
% imagesc(Data.time,1:nTrials,Data.motionEnergy(:,sortix)')
% ax.YDir = 'normal';
...
% ax.TickLength = ax.TickLength .* 2;
% hold on;
% imagesc(Data.time,1:nTrials,plt.potent.ssm(:,sortix)')
% ax.YDir = 'normal';
% line([0 0], [ax.YLim(1) ax.YLim(2)],'color','w','linestyle','--') % go cue
% title('Potent')
% xlim(xlims);
...
% ax.TickLength = ax.TickLength .* 2;
% hold on;
% imagesc(Data.time,1:nTrials,plt.null.ssm(:,sortix)')
% ax.YDir = 'normal';
% line([0 0], [ax.YLim(1) ax.YLim(2)],'color','w','linestyle','--') % go cue
% title('Null')
% xlim(xlims);
...
% figure 
% subplot(121)
% plot(Data.time,mean(plt.null.ssm,2))
% hold on
% plot(Data.time,mean(plt.potent.ssm,2))
% legend({'mov-null';'mov-potent'})
% subplot(122)
% plot(Data.time,nanmean(Data.motionEnergy,2),'k')
% hold on
% plot(Data.time,0*mean(Data.motionEnergy,2),'-.k')
% 
% prettify_plot('LineThickness', 2,'AxisTightness', 'keep','TickLength',[.02 .001])
% 
