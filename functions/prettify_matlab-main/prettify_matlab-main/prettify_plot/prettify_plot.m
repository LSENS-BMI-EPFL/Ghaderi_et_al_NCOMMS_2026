...
options = struct('XLimits', 'tight', ... % set to 'keep' if you don't want any changes
    'YLimits', 'tight', ... % set to 'keep' if you don't want any changes
...
for iAx = 1:size(all_axes, 2)
    ...
    if ~isempty(currAx)
        ...
        for thisLine = childLines'
            ...
            % adjust markersize
             if ~strcmp(options.PointSize,'keep')
            ...
            if strcmp(options.LineThickness, 'keep')
            else
        ...
        for thisText = childTexts'
            % set(thisText, 'FontSize', options.GeneralFontSize, 'Color', mainColor); % here i changed the code to avoid changing legend text color 
            set(thisText, 'FontSize', options.GeneralFontSize);
        end
