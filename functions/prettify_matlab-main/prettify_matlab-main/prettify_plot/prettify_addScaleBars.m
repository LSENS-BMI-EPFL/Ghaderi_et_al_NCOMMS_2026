function prettify_addScaleBars(xLength, yLength, labelX, labelY, position ,unitX, unitY, scaleLineWidth, fontSize,ax)
...
hold on;
h1 = plot(ax,[xBarStart, xBarEnd], [yBarStart, yBarStart], [thisColor '-'], ...
    'LineWidth', scaleLineWidth);
h2 = plot(ax,[xBarStart, xBarStart], [yBarStart, yBarEnd], [thisColor '-'], ...
    'LineWidth', scaleLineWidth);
hold off;
...
switch position
    case 'bottomRight'
        text(ax,(xBarStart + xBarEnd)/2, yBarStart-0.02*range(yLim)*sign(yLength), labelX, ...
            'HorizontalAlignment', textPos{1, 1}, 'VerticalAlignment', textPos{1, 2}, ...
            'Color', thisColor, 'FontSize', fontSize);
        text(ax,xBarStart+0.06*range(xLim)*sign(xLength), (yBarStart + yBarEnd)/2, labelY, ...
    ...
    case 'bottomLeft'
        text(ax,(xBarStart + xBarEnd)/2, yBarStart-0.02*range(yLim)*sign(yLength), labelX, ...
            'HorizontalAlignment', textPos{1, 1}, 'VerticalAlignment', textPos{1, 2}, ...
            'Color', thisColor, 'FontSize', fontSize);
        text(ax,xBarStart-0.05*range(xLim), (yBarStart + yBarEnd)/2, labelY, ...
            'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Rotation', 90, ...
            'Color', thisColor, 'FontSize', fontSize);

    case 'topRight'
        text(ax,(xBarStart + xBarEnd)/2, yBarStart+0.03*range(yLim)*sign(yLength), labelX, ...
            'HorizontalAlignment', textPos{1, 1}, 'VerticalAlignment', textPos{1, 2}, ...
            'Color', thisColor, 'FontSize', fontSize);
        text(ax,xBarStart+0.06*range(xLim)*sign(xLength), (yBarStart + yBarEnd)/2, labelY, ...
            'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Rotation', 90, ...
