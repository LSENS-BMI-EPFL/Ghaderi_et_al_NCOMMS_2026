%% =========================================================================
% Ghaderi2025_Figure6C_temporalCorrelation.m
% =========================================================================
%
% This script generates Figure 6C showing temporal correlation analysis with 10ms bins
...
% brain areas and trial conditions using 10ms time bins. It creates correlation matrices
% and plots average correlation values for different trial types.
%
% Dependencies:
%   - psth_10ms.mat (contains trial data)
%   - Temporal_corrrelation_10ms.mat (contains correlation data)
...
change_name = 0;
newname = 'Figure6C_temporalCorrelation';
...
load([directory filesep 'processed_data' filesep 'Temporal_corrrelation_10ms.mat'])
...
trialtype_names_tag = {'Hit','Nogo CR','No Whisker CR','Miss'};
    ...
    for i_cond = 1:length(trialtype_names)
        ...
        % Process correlation data
            ...
            for i_session = 1:size(curr_cond_area_correlation,3)
                ...
                auc = nanmean(curr_map(100:200,100:200),"all");
                AUC(i_session) = auc;
...
%         end % End of Pvalue condition
        ...
        L = 100;
        [lags, mean_correlations, sem_correlations] = compute_mean_corr(mean_corr_map, 100, 199, L);
        lags = lags(L:end);
    ...
    ylim(axs(ind_figures(5,ind_area)),[-0.05,0.2]);
    ...
    prettify_pvalues(axs(ind_figures(5,ind_area)), [1,1,1], [2,3,4], P_Corr(2:end),'PlotNonSignif', false,'OnlyStars',true,'Yposition',.15);
...
ylabel(h(21),'Mean delay correlation');
colorbar(axs(1),'Position',[0.165 0.78 .008 .05]);
legend_just_txt(axs(ind_figures(5,1)),trialtype_names_tag,'Xoffset',1,'Yoffset',.2,'relX',0,'relY',0.08,'type','bar');

%% Apply final plot formatting

prettify_plot('LineThickness', 1,'AxisTightness', 'keep','TickLength',[.03 .1],'PointSize','keep','GeneralFontSize',8);
