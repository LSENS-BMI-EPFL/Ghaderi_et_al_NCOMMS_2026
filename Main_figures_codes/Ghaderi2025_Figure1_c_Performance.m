%% =========================================================================
% Ghaderi2025_Figure1_performance.m
% =========================================================================
%
% This script generates Figure 1 showing performance analysis
...
% Date: 2025
%
% Description: This script analyzes and plots performance metrics (probability
% of licking) across different trial types. The analysis includes all trial
% types and calculates mean performance with standard error across sessions.
%
% Dependencies:
...
%
% Output: PDF figure showing performance analysis with proper formatting
% =========================================================================
...
for icond = 1:length(params.TrialType)
    ...
    errorbar(axs(1), icond, nanmean(performance(icond, :)), nanstd(performance(icond, :)) / sqrt(length(performance(icond, :))), 'vertical', 'Color', [0 0 0], 'CapSize', 10);

    % Store mean and standard error values
    mean_std_values(icond, :) = [nanmean(performance(icond, :)), nanstd(performance(icond, :)) / sqrt(length(performance(icond, :)))];

end % End condition loop

%% Plot individual session data
plot(axs(1), performance, 'o', 'color', [0.5 0.5 0.5]);
...
ylabel(axs(1), 'Probability of licking (%)');

% Set y-axis limits and ticks
ylim(axs(1), [0 100]);
yticks(axs(1), [0 20 40 60 80 100]);
yticklabels(axs(1), [0 20 40 60 80 100]);
