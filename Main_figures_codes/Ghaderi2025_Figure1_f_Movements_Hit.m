%% =========================================================================
% Ghaderi2025_Figure1_HitMovementTraces.m
% =========================================================================
%
% This script generates Figure 1 showing hit movement traces
...
% Date: 2025
%
% Description: This script plots movement traces (whisker speed, jaw movement,
% and tongue movement) for hit trials across multiple brain regions. The
% traces are averaged across sessions and trials, with baseline subtraction
% and error shading for visualization.
%
% Dependencies:
...
%
% Output: PDF figure showing movement traces with proper formatting
% =========================================================================
...
h = tight_subplot(1, 1, [.15 .15], [.1 .1], [.1 .1]);
...
t_start = 0.8;  % Start time (seconds)
t_end = 1.2;    % End time (seconds)
bin_width = 0.01;

% X-axis formatting
XTickLabel = {'0.9'; '1'; '1.2'};
xtick = [0.9; 1; 1.2];

%% Movement signal parameters
movements_signals = {'whisker_speed', 'jaw_movement', 'tongue_movement'};  % Signals to plot
movements_signals_tag = {'Whisker', 'Jaw', 'Tongue'};
...
% Color scheme
colors=[1 0 1; 0 0.8 1;  1 0.5 0];

% Y-axis shift for different signals
shift_y_values = [0 0 0];
...
%             Concatsig = [Concatsig, CurrSignal_CurrTrialInd];
        ...
        if params.movement_scaling
            ...
            Mov_Amp= max(meansig)-min(meansig);
            meansig=meansig./Mov_Amp;
            semsig=semsig./Mov_Amp;
    ...
    % Add signal label
    text(axs(1), 0.95, 0.9-ind_signal*0.1, movements_signals_tag{1,ind_signal}, 'FontSize', 16, 'Color', colorcode);
end % End signal loop
...
xline(axs(1), 1);
plot(h(1), [1.03 1.03], [-0.2 1.2], '--', 'color', [0.5 0.5 0.5], 'linewidth', 1);

% Set axis limits and labels
xlim(axs(1), [.9, 1.2]);
xlabel(h(1), 'Time from whisker stimulus (ms)');
...
% Add legend
legend_just_txt(axs(1), {'whisker\_speed', 'jaw\_movement', 'tongue\_movement'}, ...
    'Xoffset', 1, 'Yoffset', 8, 'relX', 0, 'relY', 0.02, 'type', 'line');

% Apply plot formatting
prettify_plot('LineThickness', 1, 'AxisTightness', 'keep', 'TickLength', [.01 .003], ...
