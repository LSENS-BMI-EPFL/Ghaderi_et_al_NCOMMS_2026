...
% responses using 10ms time bins. It performs cross-validation, orthogonalizes
...
params.preTime = -1;
params.postTime = 2;
params.BinSize = 0.01; % 0.005 ????
params.BinStep = 0.01; % 0.005 ????
...
params.Win_stim = [1, 1.03];       % Stimulus coding direction window
...
expr3 = 'Ind.Class3 = [trial == 5 & lick == 0];';     % Whisker only, no lick

% Lick classification (all trials)
expr4 = 'Ind.Class4 = [(lick == 1)];';                % All lick trials
expr5 = 'Ind.Class5 = [(lick == 0)];';                % All no-lick trials
...
for ind_area = 1:length(params.regionlist)
    ...
    for i_prob = probe_list
        ...
        % Lick coding direction (1.4-1.6s)
        ...
        % Stimulus coding direction (1.0-1.03s vs baseline -1.0-0s)
...
save([directory 'Coding_direction_10ms.mat'], "coding_direction_matrix", "windowCenters", "params");
