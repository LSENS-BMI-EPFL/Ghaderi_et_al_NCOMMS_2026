%% =========================================================================
% psth_mat_sub_PG082_ses_20221113T145317.m
% =========================================================================
% 
% This script processes electrophysiological data from a specific session
% (PG082, session 20221113T145317) for the manuscript "Contextual gating 
% of whisker-evoked responses by frontal cortex supports flexible decision making" 
% (Parviz Ghaderi, Sylvain Crochet, Carl Petersen, 2025)
% 
% Code Author: Parviz Ghaderi
% Email: parviz.ghaderi7@gmail.com
% Date: 2025
%
% Description: This script loads and processes electrophysiological data from
% NWB files for a specific experimental session. It extracts unit information,
% behavioral data, movement signals, and calculates PSTHs (Peri-Stimulus Time
% Histograms) for neural responses. The script processes movement signals
% including whisker angle, jaw movement, and tongue movement, and identifies
% quiet trials for analysis.
%
% Dependencies: 
%   - matnwb package (for reading NWB files)
%   - PSTHBehavior.m (for behavioral PSTH calculation)
%   - nwb_findQuietTrial.m (for quiet trial identification)
%   - trial_typeMaker.m (for trial type classification)
%   - PSTH_Simple.m (for spike PSTH calculation)
%   - NWB files in ../data_electrophysiology/ directory
%
% Input: NWB file containing electrophysiological and behavioral data
% Output: Processed session data saved to psth_sub_PG082_ses_20221113T145317.mat
% =========================================================================

%% Initialize workspace and load required packages
% Load the matnwb package

clear all
close all
clc


%% Define data directory and parameters
% Directory containing NWB files

CurrentDir=pwd;
directory=[CurrentDir filesep 'data_electrophysiology' filesep];

% PSTH calculation parameters
[pre_time, post_time, bin_width, bin_step] = deal(-1, 2, 0.001, 0.001);

% Initialize probe counter
Iprb = 1;

% Initialize dataset structure
psth_mat_session = [];

%% Process NWB files
% Get list of all NWB files in the directory
nwb_file_address = dir([directory '\*.nwb']);

% Process specific NWB file (index 24)
for ind_nwb = 24
    curr_nwbfile = (nwb_file_address(ind_nwb).name);
    
    % Read current NWB file
    nwb = nwbRead([nwb_file_address(ind_nwb).folder '\' nwb_file_address(ind_nwb).name]);
    
    %% Extract unit information
    % Get unit tables
    units_table = [];
    for ind_probe = 1:nwb.units.id.data.dims
        units_table = [units_table; nwb.units.getRow(ind_probe)];
    end
    
    %% Extract electrode information
    % Get electrode table
    electrodes_table = nwb.general_extracellular_ephys_electrodes.toTable;
    
    %% Extract trial information
    % Get trial table
    trials_table = nwb.intervals_trials.toTable;
    
    %% Extract behavioral time series data
    % Get behavioral time series names
    beh_timeseriesName = nwb.processing.get('behavior').nwbdatainterface.get('BehavioralTimeSeries').timeseries.keys;
    
    % Extract each behavioral time series
    for ind_probe = beh_timeseriesName
        currname = cell2mat(ind_probe);
        eval(['behavior_strc.' currname '=nwb.processing.get("behavior").nwbdatainterface.get("BehavioralTimeSeries").timeseries.get("' currname '").data.load;']);
    end
    
    %% Extract video and piezo timestamps
    % Get time stamps of video
    behavior_strc.video_timestamp = nwb.processing.get("behavior").nwbdatainterface.get("BehavioralTimeSeries").timeseries.get("C2Whisker_Angle").timestamps.load;
    
    % Get time stamps of piezo sensor
    behavior_strc.piezo_timestamp = nwb.processing.get("behavior").nwbdatainterface.get("BehavioralTimeSeries").timeseries.get("Piezo_lick_trace").timestamps.load;
    
    %% Extract behavioral events
    % Get behavioral event names
    beh_eventsName = nwb.processing.get('behavior').nwbdatainterface.get("BehavioralEvents").timeseries.keys;
    
    % Extract each behavioral event
    for ind_probe = beh_eventsName
        currname = cell2mat(ind_probe);
        eval(['behavior_strc.' currname '=nwb.processing.get("behavior").nwbdatainterface.get("BehavioralEvents").timeseries.get("' currname '").timestamps.load;']);
    end
    
    %% Standardize behavioral data structure
    % Ensure all fields are column vectors
    for ind_probe = fieldnames(behavior_strc)'
        currname = cell2mat(ind_probe);
        if size(behavior_strc.(currname), 1) < size(behavior_strc.(currname), 2)
            behavior_strc.(currname) = behavior_strc.(currname)';
        end
    end
    
    %% Process movement signals
    % Extract movement signals per trials
    Jaw_Coordinate = behavior_strc.Jaw_Coordinate;
    Tongue_Coordinate = behavior_strc.Tongue_Coordinate;
    snout_angle = behavior_strc.Snout_Angle;
    whisker_angle = behavior_strc.C2Whisker_Angle;
    video_timestamp = behavior_strc.video_timestamp;
    piezo_lick_trace = behavior_strc.Piezo_lick_trace;
    piezo_timestamp = behavior_strc.piezo_timestamp;
    
    % Filter piezo signal to remove noise
    piezo_lick_trace = sgolayfilt(double(piezo_lick_trace), 5, 11);
    
    %% Calculate movement metrics
    % Whisker signal: calculate speed from angle differences
    whisker_speed = [abs(diff(whisker_angle, 1, 1))];
    whisker_speed = [whisker_speed(1); whisker_speed];
    
    % Jaw signal: calculate movement magnitude from coordinate changes
    jawcordX = Jaw_Coordinate(:, 1);
    jawcordY = Jaw_Coordinate(:, 2);
    jaw_movement = sqrt((jawcordY - mode(jawcordY)).^2 + (jawcordX - mode(jawcordX)).^2);
    
    % Tongue signal: calculate movement magnitude from coordinate changes
    TonguecordY = Tongue_Coordinate(:, 2);
    TonguecordX = Tongue_Coordinate(:, 1);
    tongue_movement = sqrt((TonguecordY - mode(jawcordY)).^2 + (TonguecordX - mode(jawcordX)).^2);
    
    % Remove outliers about tongue movements
    tongue_movement(20 * mean(tongue_movement) < tongue_movement) = nan;  % Remove outliers
    
    %% Organize movement signals
    MovementSignal.whisker_angle = whisker_angle;
    MovementSignal.whisker_speed = whisker_speed;
    MovementSignal.jaw_movement = jaw_movement;
    MovementSignal.snout_angle = snout_angle;
    MovementSignal.tongue_movement = tongue_movement;
    
    %% Calculate PSTHs for behavioral data
    % Determine frame rate based on file name
    if any(strfind(curr_nwbfile, 'PG019'))
        frame_rate = 100;
    else
        frame_rate = 200;
    end
    
    % Calculate behavioral PSTH
    [PSTH_Behavior, WindowCenters] = PSTHBehavior(MovementSignal, video_timestamp, trials_table.start_time, pre_time, post_time, 0.01, 0.01, frame_rate);
    
    % Calculate piezo PSTH
    Piezo.piezo_lick_trace = piezo_lick_trace;
    [PSTH_Piezo, WindowCentersPiezo] = PSTHBehavior(Piezo, piezo_timestamp, trials_table.start_time, pre_time, post_time, 0.01, 0.01, 2000);
    
    %% Combine behavioral data
    behavior_table = PSTH_Behavior;
    behavior_table.piezo_lick_trace = PSTH_Piezo.piezo_lick_trace;
    behavior_table.trial_timestamps = WindowCenters.whisker_angle';
    
    %% Identify quiet trials
    % Parameters for quiet trial identification
    params.prewhisk_window = [0.8, 1];
    params.baseline_window = [-1, 0];
    params.movement_signals = {'whisker_speed', 'jaw_movement'};
    params.selectin_method = 'mad_all';   % Options: 'mad_all' or 'one_by_one'
    
    % Find quiet trials
    behavior_table = nwb_findQuietTrial(behavior_table, params);
    
    %% Process data for each brain region
    % Determine how many regions were recorded in this session
    [arean_name, bind, cind] = unique(units_table.location);
    
    for iarea_name = 1:length(arean_name)
        % Get current area name
        currarea_name = cell2mat(arean_name(iarea_name));
        ind_currarea_name = cind == iarea_name;
        
        %% Store session and probe information
        psth_mat_session(Iprb).session_id{:} = nwb.identifier;
        psth_mat_session(Iprb).probe_location = arean_name(iarea_name);
        
        %% Extract unit information
        % Get column names for unit data (excluding last 3 columns)
        columnname = fieldnames(units_table);
        columnname = columnname(1:end-3);
        
        % Store unit parameters
        for ifield = columnname'
            currfield = cell2mat(ifield);
            psth_mat_session(Iprb).(['unit_' currfield]) = [units_table.(currfield)(ind_currarea_name)];
        end
        
        %% Extract electrode information
        % Find electrodes in current area
        ind_currarea_name_elec = find([strcmp(electrodes_table.location, currarea_name)]);
        
        % Get column names for electrode data (excluding last 3 columns)
        columnname_elec = fieldnames(electrodes_table);
        columnname_elec = columnname_elec(1:end-3);
        
        % Store electrode parameters
        for ifield = columnname_elec'
            currfield = cell2mat(ifield);
            psth_mat_session(Iprb).(['elec_' currfield]) = [electrodes_table.(currfield)(ind_currarea_name_elec)];
        end
        
        %% Store behavioral data
        % Get column names for behavioral data
        columnname_behavior = fieldnames(behavior_table);
        
        % Store behavioral parameters
        for ifield = columnname_behavior'
            currfield = cell2mat(ifield);
            psth_mat_session(Iprb).(currfield) = [behavior_table.(currfield)];
        end
        
        %% Store trial information
        % Get column names for trial data (excluding last 3 columns)
        columnname_trial = fieldnames(trials_table);
        columnname_trial = columnname_trial(1:end-3);
        
        % Store trial parameters
        for ifield = columnname_trial'
            currfield = cell2mat(ifield);
            psth_mat_session(Iprb).(currfield) = [trials_table.(currfield)(:)];
        end
        
        %% Add trial type classification
        % Add trial_type column to structure if it is not in NWB files
        psth_mat_session(Iprb).trial_type = [trial_typeMaker(psth_mat_session(Iprb).whisker_stim, psth_mat_session(Iprb).context)]';
        
        %% Calculate spike counts per trial per cell
        % Get current spike times
        currspiketimes = psth_mat_session(Iprb).unit_spike_times;
        spikeCountsAll = [];
        
        % Calculate PSTH for each cluster
        for iCluster = 1:numel(currspiketimes)
            [spikeRates, Windowcenters, spikeCounts] = PSTH_Simple(currspiketimes{iCluster, 1}, psth_mat_session(Iprb).start_time, pre_time, post_time, bin_width, bin_width);
            spikeCountsAll(:, :, iCluster) = spikeCounts;
        end
        
        %% Store final data
        % Store timestamps and spike counts
        psth_mat_session(Iprb).trial_timestamps = Windowcenters';
        psth_mat_session(Iprb).behaviour_timestamps = WindowCentersPiezo.piezo_lick_trace';
        psth_mat_session(Iprb).spike_counts = spikeCountsAll;
        
        % Increment probe counter
        Iprb = Iprb + 1;
    end
end

%% Save processed data
% Save the processed session data to a MAT file

directory=[CurrentDir filesep 'processed_data' filesep];
save([directory 'psth_sub_PG082_ses_20221113T145317.mat'], "psth_mat_session", '-v7.3');

