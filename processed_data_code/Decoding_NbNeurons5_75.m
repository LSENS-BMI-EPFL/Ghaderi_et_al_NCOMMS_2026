%% =========================================================================
% Decoding_NbNeurons5_75.m
% =========================================================================
% 
% This script performs decoding analysis with varying numbers of neurons
% (5 to 75) for the manuscript "Contextual gating of whisker-evoked 
...
% with systematically varying numbers of neurons (5, 10, 15, ..., 75).
% It randomly samples different subsets of neurons and repeats the analysis
% 100 times for each neuron count to assess the robustness of decoding
% performance as a function of population size. This version extends the
% analysis to larger population sizes compared to Decoding_NbNeurons5_50.m.
%
% Dependencies: 
...
% Output: Decoding accuracy results saved to Decoding_NbNeurons5_75.mat
% =========================================================================
...
for ind_area = 1:length(params.regionlist)
    ...
    for ind_probe = list_probes
        ...
        Currsig_CurrCellInd_all = Currsig(:, :, CurrCellInd);

        %% Analyze different numbers of neurons
        NbNeurons_list = [5:5:75];  % Neuron counts: 5, 10, 15, ..., 75

        for i_Nb = NbNeurons_list
            commonFeatureSize = i_Nb;
            name = ['Nb_neurons' num2str(i_Nb)];
            numRepetitions = 100;  % Number of random neuron selections
            
            % Skip if insufficient neurons available
            if size(Currsig_CurrCellInd_all, 3) < i_Nb
                continue
            end
            
            selectedNeurons_mat = [];
            
            %% Perform multiple repetitions with random neuron selection
            for i_repetition = 1:numRepetitions
                i_repetition;
                
                % Randomly sample neurons
                rng(i_repetition);
                selectedFeatures = randperm(size(Currsig_CurrCellInd_all, 3), commonFeatureSize);
                selectedNeurons_mat(i_repetition, :) = selectedFeatures;
                Currsig_CurrCellInd = Currsig_CurrCellInd_all(:, :, selectedFeatures);
                ...
                for iBin = 1:size(Value.Class1(:, :, :), 1)
                    ...
                    if (size(X1, 2) < params.mintrial || size(X2, 2) < params.mintrial)
                        Accuracy.(name).(currentarea)(session_counter, iBin) = nan;
                        continue
                    end
                    ...
                    %% Store results
                    mean(acc);
                    Accuracy.(name).(currentarea)(session_counter, iBin, i_repetition) = mean(acc);
                    Accuracy_shuffeled.(name).(currentarea)(session_counter, iBin, i_repetition) = mean(acc_shuffeled);
                    Accuracy.(name).sessionaddress.(currentarea)(session_counter) = psth_mat(ind_probe).session_id;
                    Accuracy_shuffeled.(name).sessionaddress.(currentarea)(session_counter) = psth_mat(ind_probe).session_id;
                end % End of time bin loop
                
                % Display progress
                ['area: ' currentarea ' ' num2str(session_counter) '/' num2str(length(list_probes)) ' ' name ' repetition ' num2str(i_repetition)];
            end % End of repetition loop
            
            %% Store selected neurons information
            Accuracy.(name).selected_neurons.(currentarea){session_counter} = selectedNeurons_mat;
...
save([directory 'Decoding_NbNeurons5_75.mat'], "Accuracy", "Accuracy_shuffeled", "windowCenters");
