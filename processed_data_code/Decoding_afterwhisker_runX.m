%% =========================================================================
% Decoding_afterwhisker_runX.m
% =========================================================================
% 
% This script performs decoding analysis after whisker stimulation with
% multiple runs for the manuscript "Contextual gating of whisker-evoked 
...
% with multiple runs (1-5) to assess decoding performance after whisker
% stimulation. It analyzes neural responses in a specific time window
% (391:420 bins) and includes baseline subtraction options for data
% normalization. Each run uses a different random seed for reproducibility.
%
% Dependencies: 
...
%
% Input: Processed PSTH data with 5ms bins
% Output: Decoding accuracy results saved to Decoding_afterwhisker_runX.mat
%         for each run (X = 1-5)
% =========================================================================
...
%% Main analysis loop across multiple runs
for run = 1:5
    %% Initialize run-specific workspace
    clearvars -except psth_mat run area_list
    ...
    params.BaselineSubtraction = 1;  % 1: subtract baseline, 0: no baseline subtraction
    ...
    params.preTime = -1;
    params.postTime = 2;
    params.BinSize = 0.005;  % 5ms bins
    params.BinStep = 0.005;  % 5ms step
    params.normalization = 0;
    params.zscoring = 1;  % Do zscore on feature matrix
    params.Separation = 0;
    params.spikecount = 1;    % 1: spike count, 0: rate
    params.windowCenters = [-1 + 0.005:0.005:2]';   % Time window centers

    %% Define classification expressions and baseline parameters
    % Binary classification between two trial conditions
    baselinelist = [0.955:0.005:1];  % Baseline time windows for subtraction
    ...
    for ind_area = 1:length(params.regionlist)
        ...
        for ind_probe = list_probes
            ...
            %% Analyze different baseline windows
            for ibase = 1:length(baselinelist)
                ...
                if params.BaselineSubtraction
                    t1 = baselinelist(ibase);
                    t2 = baselinelist(ibase);
                    [a, b] = min(abs(params.windowCenters - t1)); baselineFirstBin = (b);
                    [a, b] = min(abs(params.windowCenters - t2)); baselineLastBin = (b);
                    
                    for neuron = 1:size(Currsig_CurrCellInd, 3)
                        % Extract data for this neuron: 600 time bins x 18 trials
                        neuron_data = Currsig_CurrCellInd(:, :, neuron);
                        % Get the submatrix for the specified time bins
                        sub_data = neuron_data(baselineFirstBin:baselineLastBin, :);  % size: (end_bin-start_bin+1) x 18
                        % Compute mean across both time bins and trials
                        baseline_mean = mean(sub_data, 1);  % scalar value
                        % Subtract this mean from the entire 600x18 matrix for this neuron
                        normalized_data(:, :, neuron) = neuron_data - repmat(baseline_mean, 600, 1);
                    end
                    Currsig_CurrCellInd = normalized_data;
                end

                %% Extract specific time window for analysis
                % Focus on time window 391:420 (specific post-whisker period)
                Currsig_CurrCellInd = Currsig_CurrCellInd(391:420, :, :);
                ...
                for iBin = 1:size(Value.Class1(:, :, :), 1)
                    ...
                    %% Store results
                    mean(acc);
                    Accuracy.(currentarea)(session_counter, iBin, ibase) = mean(acc);
            ...
            end % End of baseline loop
        end % End of probe loop
    end     % End of area loop
    
    %% Prepare output variables and save results
    windowCenters = params.windowCenters(391:420);
%     save(['../processed_data/Decoding_afterwhisker_run' num2str(run) '.mat'], "Accuracy", "Accuracy_shuffeled", "windowCenters");
    CurrentDir=pwd;
    directory=[CurrentDir filesep 'processed_data' filesep];
    save([directory 'Decoding_afterwhisker_run' num2str(run) '.mat'], "Accuracy", "Accuracy_shuffeled", "windowCenters");
